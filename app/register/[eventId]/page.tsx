...
export default function RegisterPage({ params }: Props) {
  ...
  if (authState === 'loading') {
    return (
      <Centered>
        <p className="font-display text-3xl tracking-widest text-[#0A2540] animate-pulse">CARGANDO…</p>
      </Centered>
    )
  }
  if (authState === 'invalid') {
    return (
      <Centered>
        <p className="font-display text-4xl tracking-widest text-[#C2410C]">LINK INVÁLIDO</p>
  ...
  return (
    ...
    >
      {showSuccessSplash && (
        <div className="fixed inset-0 z-[9999] bg-[#059669] text-white flex flex-col items-center justify-center p-6" style={{ animation: 'fadeIn 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}>
      ...
      {!(step.kind === 'welcome' || step.kind === 'instruction_1' || step.kind === 'instruction_2') && (
        ...
        {canBack && step.kind !== 'summary' ? (
          <button
            onClick={goBack}
            className="shrink-0 inline-flex items-center justify-center text-[#0A2540] active:opacity-60 h-10 w-10 -ml-2 rounded-full active:scale-95 transition-all duration-150"
            aria-label="Atrás"
          >
        ...
        <div className="min-w-0 flex-1 flex items-center gap-2">
          <div className="truncate">
            <p className="font-display text-[9px] tracking-[0.2em] text-[#0A2540] leading-none">REGISTRO PARA</p>
            <p className="font-display text-xs uppercase text-[#1A1D1E] truncate leading-tight mt-0.5">
          ...
          {step.kind === 'dancer' && (
            <button
              ...
              className="shrink-0 inline-flex items-center gap-1 bg-[#059669]/10 text-[#059669] border border-[#059669]/30 px-2.5 py-1 rounded-full font-display text-[10px] tracking-wider font-bold active:scale-95 transition-all duration-150"
            >
      ...
      >
        ...
        <div className="shrink-0 hidden lg:flex items-center gap-6 pb-4">
          <div className="shrink-0 flex items-baseline gap-5">
            <p className="font-display text-3xl lg:text-4xl tracking-[0.3em] text-[#0A2540] leading-none">REGISTRO PARA</p>
          ...
          {(step.kind === 'summary' || step.kind === 'confirmed') && (
            <div className="text-right self-center shrink-0">
              <h2 className="font-display text-3xl lg:text-4xl uppercase text-[#0A2540] leading-tight">
                {step.kind === 'confirmed' ? 'REGISTRO CONFIRMADO' : 'REVISA TU REGISTRO'}
              </h2>
              <p className="font-display text-base lg:text-lg tracking-[0.4em] text-[#0A2540]/70 leading-none mt-2">
                {step.kind === 'confirmed' ? '¡GRACIAS!' : editMode ? 'EDITA Y VUELVE A CONFIRMAR' : 'SI TODO ES CORRECTO, CONFIRMA'}
              </p>
            </div>
          )}
          {editMode && (
            <div className="bg-[#C2410C]/10 border border-[#C2410C]/30 text-[#C2410C] px-3 py-1.5 rounded-xl font-display text-xs tracking-widest self-center">
              MODO EDICIÓN
            </div>
          )}
        ...
        {step.kind === 'summary' ? (
          <div className="px-6 pt-3 pb-4 shrink-0 flex items-center justify-center">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-3 text-[#334155] hover:text-[#0A2540] transition-all group active:scale-98"
            >
              <MessageCircle className="w-5 h-5 text-[#059669] group-hover:text-[#164235] shrink-0" />
              <span className="text-sm md:text-base">
                ¿Dudas o ayuda? Escríbenos por WhatsApp:{' '}
                <span className="font-display tracking-wider text-[#0A2540] group-hover:text-[#020B14]">333 729 0374</span>
              </span>
            </a>
          </div>
        ) : (
          <div className="px-6 pt-3 pb-6 shrink-0 flex items-center gap-4">
            <div className="min-w-0 flex-1 flex justify-start">
              {canBack && (
                <button
                  onClick={goBack}
                  className="inline-flex items-center gap-3 bg-[#E8E3D5] hover:bg-[#DDD8CA] active:bg-[#C2BCB0] text-[#0A2540] font-display text-base tracking-[0.2em] uppercase px-4 py-2 rounded-xl transition-all border border-[#C2BCB0]/50 active:scale-95"
                >
      ...
      {mobileSheet === 'dancers' && (
        <div className="lg:hidden fixed inset-0 z-50 flex items-end" onClick={() => setMobileSheet(null)}>
          ...
          <div className="relative w-full bg-[#F6F4EF] border-t-2 border-[#0A2540] rounded-t-3xl max-h-[80vh] flex flex-col shadow-[0_-8px_30px_rgba(0,0,0,0.08)]" onClick={e => e.stopPropagation()}>
            <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-[#C2BCB0]/40">
              <p className="font-display text-base tracking-[0.3em] text-[#0A2540]">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 rounded-xl flex items-center gap-3 active:scale-[0.98] transition-all duration-150 ${
                        isCurrent ? 'bg-[#0A2540] text-white shadow-md' : complete ? 'bg-[#059669]/10 text-[#059669] active:bg-[#059669]/20' : 'bg-[#C2410C]/10 text-[#C2410C] active:bg-[#C2410C]/20'
                      }`}
                    >
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setEditMenu(null)}>
          <div className="bg-[#F6F4EF] border border-[#C2BCB0] rounded-3xl max-w-2xl w-full max-h-full overflow-hidden flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="bg-[#E8E3D5] px-6 py-4 flex items-center justify-between shrink-0 border-b border-[#C2BCB0]">
              <h3 className="font-display text-2xl tracking-widest text-[#0A2540]">
...
function EditMenuButton({ label, sub, onClick }: { label: string, sub?: string, onClick: () => void }) {
  return (
    ...
    >
      <div className="font-display text-xl uppercase text-[#0A2540]">{label}</div>
...
function StepView(props: {
  ...
  switch (step.kind) {
      ...
      return (
        <div className="flex flex-col items-center justify-center text-center space-y-6 max-w-xl mx-auto py-4">
          ...
          <div className="space-y-2">
            <p className="font-display text-xs tracking-[0.4em] text-[#0A2540]">SISTEMA OFICIAL DE REGISTRO</p>
            <h2 className="font-display text-4xl lg:text-5xl uppercase text-[#1A1D1E] tracking-tight">{event?.name || 'EVENTO'}</h2>
            {event?.date && (
              <p className="font-display text-lg tracking-widest text-[#334155] uppercase">{eventCity} · {formatEventDate(event.date)}</p>
            )}
          </div>

          <div className="w-full bg-[#E8E3D5]/40 border border-[#C2BCB0]/50 rounded-2xl p-5 space-y-4 text-left shadow-sm">
            <div className="space-y-1">
              <p className="text-[10px] font-display tracking-wider text-[#334155]/70 uppercase">FECHA LÍMITE DE REGISTRO</p>
              <p className="text-base text-[#0A2540] font-semibold">{regDeadline}</p>
            </div>
            <div className="h-px bg-[#C2BCB0]/30" />
            <div className="space-y-1">
              <p className="text-[10px] font-display tracking-wider text-[#334155]/70 uppercase">FECHA LÍMITE PARA CAMBIOS</p>
              <p className="text-base text-[#C2410C] font-semibold">{chgDeadline}</p>
            </div>
          </div>

          <div className="flex items-start gap-3 bg-[#C2410C]/10 border border-[#C2410C]/30 text-[#C2410C] px-5 py-4 rounded-2xl text-left">
          ...
          <button
            onClick={onNext}
            className="w-full bg-[#0A2540] active:bg-[#020B14] hover:bg-[#020B14] text-white font-display text-xl tracking-widest py-4 rounded-2xl transition-all shadow-md active:scale-[0.98] duration-150"
          >
    ...
    case 'instruction_1': {
      return (
        <div className="flex flex-col justify-center w-full max-w-2xl mx-auto py-3 px-0 space-y-4 lg:space-y-6">
          <div className="text-center space-y-1 lg:space-y-2">
            <p className="font-display text-xs lg:text-sm tracking-[0.4em] text-[#0A2540] font-bold">PASO 1 DE 2</p>
            <h2 className="font-display text-3xl lg:text-4xl text-[#0B132B] font-bold">REGLAS Y CATEGORÍAS</h2>
          </div>

          <div className="space-y-3 lg:space-y-4">
            <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 lg:p-6 shadow-sm flex items-start gap-3.5 lg:gap-4 active:scale-[0.99] transition-all duration-150">
              <span className="shrink-0 flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-[#0A2540] text-white font-display text-base font-bold">1</span>
              <div className="space-y-1">
                <h4 className="font-display text-base lg:text-lg text-[#0A2540] tracking-wider uppercase font-bold">Categorías de Edad</h4>
                <p className="text-sm lg:text-base text-[#1E293B] leading-relaxed">
            ...
            <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 lg:p-6 shadow-sm flex items-start gap-3.5 lg:gap-4 active:scale-[0.99] transition-all duration-150">
              <span className="shrink-0 flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-[#0A2540] text-white font-display text-base font-bold">2</span>
              <div className="space-y-1">
                <h4 className="font-display text-base lg:text-lg text-[#0A2540] tracking-wider uppercase font-bold">Integrantes del Equipo</h4>
                <p className="text-sm lg:text-base text-[#1E293B] leading-relaxed">
            ...
            <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 lg:p-6 shadow-sm flex items-start gap-3.5 lg:gap-4 active:scale-[0.99] transition-all duration-150">
              <span className="shrink-0 flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-[#0A2540] text-white font-display text-base font-bold">3</span>
              <div className="space-y-1">
                <h4 className="font-display text-base lg:text-lg text-[#0A2540] tracking-wider uppercase font-bold">Nombre de Equipo</h4>
                <p className="text-sm lg:text-base text-[#1E293B] leading-relaxed">
          ...
          <button
            onClick={onNext}
            className="w-full bg-[#0A2540] active:bg-[#020B14] hover:bg-[#020B14] text-white font-display text-lg lg:text-xl tracking-widest py-4 rounded-2xl transition-all shadow-md active:scale-[0.98] duration-150 font-bold"
          >
    ...
    case 'instruction_2': {
      return (
        <div className="flex flex-col justify-center w-full max-w-2xl mx-auto py-3 px-0 space-y-4 lg:space-y-6">
          <div className="text-center space-y-1 lg:space-y-2">
            <p className="font-display text-xs lg:text-sm tracking-[0.4em] text-[#0A2540] font-bold">PASO 2 DE 2</p>
            <h2 className="font-display text-3xl lg:text-4xl text-[#0B132B] font-bold">ACTOS Y COSTOS</h2>
          </div>

          <div className="space-y-3 lg:space-y-4">
            <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 lg:p-6 shadow-sm flex items-start gap-3.5 lg:gap-4 active:scale-[0.99] transition-all duration-150">
              <span className="shrink-0 flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-[#0A2540] text-white font-display text-base font-bold">4</span>
              <div className="space-y-1">
                <h4 className="font-display text-base lg:text-lg text-[#0A2540] tracking-wider uppercase font-bold">Registro de Actos</h4>
                <p className="text-sm lg:text-base text-[#1E293B] leading-relaxed">
            ...
            <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 lg:p-6 shadow-sm flex items-start gap-3.5 lg:gap-4 active:scale-[0.99] transition-all duration-150">
              <span className="shrink-0 flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-[#0A2540] text-white font-display text-base font-bold">5</span>
              <div className="space-y-1">
                <h4 className="font-display text-base lg:text-lg text-[#0A2540] tracking-wider uppercase font-bold">Cálculo de Costos</h4>
                <p className="text-sm lg:text-base text-[#1E293B] leading-relaxed">
            ...
            <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 lg:p-6 shadow-sm flex items-start gap-3.5 lg:gap-4 active:scale-[0.99] transition-all duration-150">
              <span className="shrink-0 flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-[#0A2540] text-white font-display text-base font-bold">6</span>
              <div className="space-y-1">
                <h4 className="font-display text-base lg:text-lg text-[#0A2540] tracking-wider uppercase font-bold">Revisión Final</h4>
                <p className="text-sm lg:text-base text-[#1E293B] leading-relaxed">
          ...
          <button
            onClick={onNext}
            className="w-full bg-[#C2410C] active:bg-[#9A3412] hover:bg-[#9A3412] text-white font-display text-lg lg:text-xl tracking-widest py-4 rounded-2xl transition-all shadow-md active:scale-[0.98] duration-150 font-bold animate-[pulse_2s_infinite]"
          >
            COMENZAR REGISTRO
          </button>
        </div>
        ...
        <Wrapper title="Nombres de los demás coaches" subtitle="Tu nombre ya está registrado" isKeyboardOpen={isKeyboardOpen}>
          <div className="space-y-3">
            {extras.map((e, i) => (
              <div key={i} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white border border-[#C2BCB0] text-[#1A1D1E] text-2xl rounded-2xl px-5 py-4 outline-none focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540] text-center transition-all shadow-sm"
                  autoCapitalize="words"
                />
                <button
                  onClick={() => updateCoach({ extras: extras.filter((_, j) => j !== i) })}
                  className="bg-[#C2410C]/10 active:bg-[#C2410C]/20 text-[#C2410C] px-4 rounded-2xl shrink-0 transition-all active:scale-95 duration-150"
                  aria-label="Quitar"
                >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-[#E8E3D5] active:bg-[#C2BCB0] text-[#0A2540] py-4 rounded-2xl font-display text-lg tracking-wider transition-all border border-[#C2BCB0]/40 active:scale-[0.98] duration-150"
            >
      ...
      return (
        <Wrapper title="¿Nombre del equipo?" isKeyboardOpen={isKeyboardOpen}>
          <input
            ...
            className="w-full bg-white border border-[#C2BCB0] text-[#1A1D1E] text-2xl lg:text-4xl rounded-2xl px-4 py-4 lg:px-6 lg:py-6 outline-none focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540] text-center placeholder:text-[#334155]/60 transition-all shadow-sm"
          />
          <p className="text-[#C2410C] text-base text-center italic font-medium">
              ...
              <div className="space-y-5 w-full">
                <div className="max-w-xs md:max-w-sm mx-auto w-full">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-[#C2BCB0] text-[#1A1D1E] text-xl rounded-2xl px-5 h-16 outline-none focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540] text-center placeholder:text-[#334155]/60 transition-all shadow-sm animate-none"
                    autoCapitalize="words"
                  />
                </div>
                <div className="max-w-xs md:max-w-sm mx-auto w-full">
                ...
                {effectiveCat && (
                  <div className="max-w-xs md:max-w-sm mx-auto w-full">
                    <label className="block text-sm font-display tracking-widest text-[#334155] mb-3 text-center">
                      CATEGORÍA {d.categoryOverride && <span className="text-[#C2410C]">· MODIFICADA</span>}
                    </label>
                    <select
                      ...
                      className={`w-full h-14 text-[#1A1D1E] text-lg text-center rounded-2xl outline-none font-display appearance-none cursor-pointer px-3 transition-all border ${
                        d.categoryOverride
                          ? 'bg-[#C2410C]/10 border-[#C2410C]/50 ring-1 ring-[#C2410C]/30'
                          : 'bg-white border-[#C2BCB0] focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540]'
                      }`}
                    >
                    ...
                    <p className="text-xs text-[#C2410C] mt-3 text-center leading-snug font-medium">
    ...
    case 'act_modality': {
      ...
      return (
        <Wrapper title={`Acto ${i + 1} de ${state.actCount}`} subtitle="MODALIDAD">
          ...
          {firstAct && (
            <div className="flex items-start gap-3 bg-[#0A2540]/10 border border-[#0A2540]/30 text-[#0A2540] px-5 py-4 rounded-2xl">
          ...
          <div className="w-full max-w-3xl mx-auto space-y-2">
            <button
              ...
              className="w-full flex items-center justify-between bg-white border border-[#C2BCB0] px-5 py-3.5 rounded-2xl font-display text-sm tracking-wider text-[#0A2540] transition-all hover:bg-[#E8E3D5]/20 active:scale-[0.99] shadow-sm duration-150"
            >
              <span>MOSTRAR TABLA DE CATEGORÍAS Y REGLAS</span>
              <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${showCategoriesInfo ? 'rotate-180 text-[#C2410C]' : ''}`} />
            </button>
            {showCategoriesInfo && (
              <div className="bg-white border border-[#C2BCB0] rounded-2xl overflow-x-auto shadow-md w-full" style={{ animation: 'fadeIn 0.25s ease-out forwards' }}>
                <table className="w-full text-left min-w-[500px]">
                  <thead>
                    <tr className="bg-[#E8E3D5] text-[#0A2540] font-display text-xs tracking-widest border-b border-[#C2BCB0]">
        ...
        <Wrapper title={`Acto ${i + 1} · ${a.modality ? modalityLabel(a.modality) : ''}`} subtitle="CATEGORÍA">
          {avgCat && (
            <p className="text-center text-[#334155] text-base">
              Categoría sugerida por edad promedio del equipo: <span className="text-[#0A2540] font-display">{AGE_CATEGORY_LABELS[avgCat].toUpperCase()}</span>
            </p>
          )}
              ...
              <button
                ...
                className={`py-7 px-3 rounded-2xl font-display tracking-wider transition-all flex flex-col items-center justify-center gap-1 border active:scale-[0.98] duration-150 ${
                  a.ageCategory === cat
                    ...
                    : cat === avgCat
                      ? 'bg-white text-[#1A1D1E] border-2 border-[#0A2540] hover:bg-[#E8E3D5]'
      ...
      return (
        <div className="flex flex-col h-auto lg:h-full max-h-full min-h-0">
          <div className="text-center space-y-3 shrink-0 mb-5">
            <p className="font-display text-xs md:text-sm tracking-[0.4em] text-[#0A2540]">
              ...
              {sortedCats.map(cat => {
                ...
                return (
                  <div key={cat} className={disabled ? 'opacity-30' : ''}>
                    <p className="font-display text-xs tracking-[0.4em] text-[#0A2540] mb-2 sticky top-0 bg-[#F6F4EF] py-1.5 z-10">
        ...
        <div className="flex flex-col h-auto lg:h-full min-h-0">
          {!isKeyboardOpen && (
            <div className="shrink-0 text-center space-y-2 lg:space-y-3 pt-2 lg:pt-0 pb-4 lg:pb-6">
              <p className="font-display text-xs lg:text-sm tracking-[0.4em] text-[#0A2540]">COSTOS</p>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pb-3 -mx-1 px-1">
            <div className={`space-y-3 lg:space-y-6 max-w-3xl mx-auto ${isKeyboardOpen ? 'pt-2' : ''}`}>
              {!isKeyboardOpen && (
                <p className="text-[#C2410C] text-sm lg:text-base italic text-center leading-snug font-medium">
              ...
              {valid && (
                <div className={`text-center bg-[#C2410C] text-white rounded-2xl shadow-sm transition-all duration-150 ${isKeyboardOpen ? 'p-2.5 mt-2.5' : 'p-4 mt-4'}`}>
  ...
  return (
    <Wrapper title={title} subtitle={hint} isKeyboardOpen={isKeyboardOpen}>
      <input
        ...
        autoCorrect="off"
        className="w-full bg-white border border-[#C2BCB0] text-[#1A1D1E] text-2xl lg:text-4xl rounded-2xl px-4 py-4 lg:px-6 lg:py-6 outline-none focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540] text-center transition-all shadow-sm"
      />
      {notice && (
        <p className="text-[#C2410C] text-sm lg:text-lg text-center italic font-medium">{notice}</p>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 lg:gap-4 max-w-xl mx-auto w-full aspect-[2/1] lg:aspect-auto">
        <button
          ...
          className={`rounded-2xl font-display text-3xl lg:text-4xl tracking-widest transition-all flex items-center justify-center lg:py-10 active:scale-95 duration-150 ${
            value === false
              ? 'bg-[#0A2540] text-white hover:bg-[#020B14] shadow-md'
...
function MoneyInput({ value, onChange, onEnter }: {
  ...
  return (
    <div className="relative max-w-sm mx-auto">
      <span className="absolute left-4 lg:left-5 top-1/2 -translate-y-1/2 text-[#0A2540] font-display text-2xl lg:text-3xl pointer-events-none">$</span>
      <input
        type="text"
        inputMode="decimal"
...
function NumberInput({ value, onChange, max, disabled, onEnter }: {
  ...
  return (
    <input
      ...
      onKeyDown={e => { if (e.key === 'Enter' && onEnter) onEnter() }}
      disabled={disabled}
      className="w-full max-w-xs mx-auto block bg-white border border-[#C2BCB0] text-[#1A1D1E] text-5xl text-center rounded-2xl h-20 outline-none focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540] font-display disabled:opacity-60 transition-all shadow-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
    />
  )
}
    ...
    <div className="relative w-full max-w-[280px] mx-auto">
      <input
        ...
        className={`w-full h-14 rounded-2xl text-center outline-none font-display text-lg tracking-wider border transition-all cursor-pointer ${
          value
            ? 'bg-white border-[#0A2540] text-[#0A2540] focus:ring-1 focus:ring-[#0A2540] shadow-sm font-semibold'
            : 'bg-white border-[#C2BCB0] text-[#334155]/60 focus:border-[#0A2540] focus:ring-1 focus:ring-[#0A2540]'
        }`}
        style={{
...
function DancerList({ dancers, currentIndex, onSelect }: {
  ...
  return (
    <div className="h-full w-full flex flex-col min-h-0">
      <p className="text-xs font-display tracking-[0.4em] text-[#0A2540] mb-4 shrink-0 text-center">
          ...
          {filled.map(({ d, i }) => {
            ...
            return (
              <button
                key={i}
                onClick={() => onSelect(i)}
                className={`text-center font-display text-2xl uppercase tracking-wide transition-all whitespace-nowrap hover:text-[#0A2540] active:scale-95 duration-150 ${
                  isCurrent ? 'text-[#0A2540] underline underline-offset-4 font-semibold' : 'text-[#059669]'
                }`}
              >
                {d.name}
              </button>
...
function EditNotice({ text }: { text: string }) {
  return (
    <p className="bg-[#C2410C]/10 border border-[#C2410C]/30 text-[#C2410C] text-center text-sm px-4 py-3 rounded-xl font-medium">
      {text}
    </p>
  )
}
  ...
  return (
    <div className="flex flex-col h-auto lg:h-full min-h-0">
      <div className="shrink-0 text-center space-y-2 lg:space-y-3 pt-2 lg:pt-0 pb-5 lg:pb-8">
        {subtitle && <p className="font-display text-xs tracking-[0.4em] text-[#0A2540]">{subtitle}</p>}
...
function NextButton({ onClick, disabled, label }: { onClick: () => void, disabled: boolean, label?: string }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-full bg-[#0A2540] active:bg-[#020B14] hover:bg-[#020B14] text-white font-display text-xl lg:text-2xl tracking-widest py-4 lg:py-5 rounded-2xl disabled:opacity-40 transition-all shadow-md active:scale-[0.98] duration-150"
    >
      {label ?? 'SIGUIENTE'}
    </button>
  )
    ...
    <div className="min-h-[100dvh] bg-[#F6F4EF] text-[#1A1D1E] flex flex-col font-sans">
      ...
      <div className="px-6 py-3 shrink-0 flex justify-end">
        ...
        >
          ...
          <span className="text-sm md:text-base">
            ¿Dudas o ayuda? Escríbenos por WhatsApp:{' '}
            <span className="font-display tracking-wider text-[#0A2540]">333 729 0374</span>
          </span>
        </a>
      </div>
    </div>
...
function MobileSummary({ state, editMode, tab, setTab, confirmed, confirm, saving, saveErr, startEdit, onEditRequest }: {
  ...
  return (
    <div className="flex flex-col h-full max-h-full min-h-0 gap-3">
      ...
      {hasCosts && (
        <div className="shrink-0 bg-[#C2410C] text-white rounded-2xl py-3 px-4 text-center shadow-sm">
        ...
        {tabs.map(t => (
          <button
            ...
            className={`py-2.5 rounded-xl font-display text-xs tracking-widest flex flex-col items-center justify-center gap-0.5 transition-all active:scale-[0.96] duration-150 ${
              tab === t.id
                ? 'bg-white text-[#0A2540] border border-[#C2BCB0]/30 shadow-sm'
                : 'text-[#334155] hover:text-[#0A2540] active:bg-[#E8E3D5]/50'
            }`}
          >
            <span>{t.label}</span>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-white border border-[#C2BCB0] rounded-2xl p-4 shadow-sm text-[#1A1D1E]">
        {tab === 'coach' && (
          <div className="space-y-4 text-center py-4 flex flex-col items-center justify-center animate-[fadeIn_0.2s_ease-out_forwards]">
            <div className="w-14 h-14 bg-[#0A2540]/10 rounded-full flex items-center justify-center text-[#0A2540] mb-1">
              <Pencil className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs tracking-widest text-[#0A2540]/70 uppercase font-bold mb-1">Nombre del Coach</p>
            ...
            <div className="space-y-2 text-base text-[#334155]">
              ...
              {state.coach.email && (
                <p className="flex items-center justify-center gap-2 text-sm text-[#334155]/90 break-all max-w-xs">
            ...
            {state.coach.extras.filter(e => e.trim()).length > 0 && (
              <div className="pt-2 border-t border-[#C2BCB0]/20 w-full max-w-xs">
                <p className="text-[10px] tracking-widest text-[#0A2540] mb-1 font-bold">OTROS COACHES</p>
        ...
        {tab === 'academy' && (
          <div className="space-y-6 text-center py-4 flex flex-col items-center justify-center animate-[fadeIn_0.2s_ease-out_forwards]">
            <div className="w-14 h-14 bg-[#C2410C]/10 rounded-full flex items-center justify-center text-[#C2410C] mb-1">
              <Check className="w-6 h-6" />
            </div>
            <div className="w-full max-w-xs">
              <p className="text-xs tracking-widest text-[#0A2540]/70 uppercase font-bold mb-1.5">Colegio / Academia</p>
              <p className="font-display text-3xl uppercase text-[#1A1D1E] break-words leading-tight">{state.academy}</p>
            </div>
            <div className="w-full max-w-xs">
              <p className="text-xs tracking-widest text-[#C2410C]/70 uppercase font-bold mb-1.5">Nombre del Equipo</p>
              <p className="font-display text-2xl uppercase text-[#059669] break-words leading-tight">{state.teamName || state.academy}</p>
            </div>
          </div>
        )}
        {tab === 'dancers' && (
            ...
            {filledDancers.length === 0 ? (
              ...
              return (
                <div key={di} className="flex items-baseline gap-2 py-1.5 border-b border-[#C2BCB0]/30 last:border-0">
                  ...
                  <span className="font-display text-base uppercase text-[#059669] flex-1 truncate font-semibold">{d.name}</span>
                  {n > 0 && <span className="text-xs text-[#0A2540] font-bold shrink-0">{n}×</span>}
                  {cost !== null && <span className="text-xs text-[#C2410C] font-bold shrink-0">{formatMoney(cost)}</span>}
                </div>
              )
            })}
          </div>
              ...
              return (
                <div key={i} className="border-b border-[#C2BCB0]/30 last:border-0 pb-2 last:pb-0">
                  <div className="font-display text-base text-[#1A1D1E] font-semibold">
                    <span className="text-[#0A2540]">#{i + 1}</span> {cat.toUpperCase()} · {mod}{lvl}
                  </div>
      ...
      {/* BUTTONS */}
      {saveErr && (
        <p className="shrink-0 text-[#C2410C] text-xs bg-[#C2410C]/10 border border-[#C2410C]/30 rounded-xl px-3 py-2.5 break-words font-medium">{saveErr}</p>
      )}
      {confirmed ? (
        <button
          onClick={startEdit}
        ...
        >
          <Pencil className="w-5 h-5 text-[#0A2540]" /> EDITAR REGISTRO
        </button>
      ) : (
        <div className="shrink-0 grid grid-cols-2 gap-2">
          ...
          >
            <Pencil className="w-5 h-5 text-[#0A2540]" /> EDITAR
          </button>
          <button
            onClick={() => {
            ...
            disabled={saving}
            className="h-14 bg-[#0A2540] hover:bg-[#020B14] active:bg-[#020B14] text-white font-display text-base tracking-widest rounded-2xl disabled:opacity-50 transition-all shadow-md active:scale-[0.98] duration-150"
          >
            {tab !== 'acts'
              ? 'SIGUIENTE'
              : saving
  ...
  return (
    <div className="lg:h-full lg:max-h-full lg:min-h-0 grid grid-cols-1 lg:grid-cols-6 lg:grid-rows-[auto_minmax(0,1fr)] gap-4 w-full">
      ...
      <div className="lg:col-span-1 lg:min-h-0">
        {hasCosts ? (
          <div className="bg-[#C2410C] text-white rounded-2xl h-full p-4 flex flex-col items-center justify-center shadow-sm">
          ...
          <div className="max-h-[60vh] lg:max-h-none lg:flex-1 lg:min-h-0 overflow-y-auto lg:overflow-x-auto lg:overflow-y-hidden">
            <div className="lg:h-full flex flex-col lg:flex-wrap lg:content-start gap-x-12 gap-y-2 px-1">
              {filledDancers.length === 0 ? (
                ...
                const cost = hasCosts && n > 0 ? (state.costPaquete ?? 0) + Math.max(0, n - 1) * (state.costRepeticion ?? 0) : null
                return (
                  <div key={di} className="font-display text-lg md:text-xl lg:text-2xl uppercase text-[#059669] whitespace-nowrap flex items-baseline gap-3">
                    <span className="opacity-40 text-sm text-[#334155]">{di + 1}.</span>
                    <span>{d.name}</span>
                    {n > 0 && <span className="text-sm text-[#0A2540] font-bold">{n} acto{n === 1 ? '' : 's'}</span>}
                    {cost !== null && <span className="text-sm text-[#C2410C] font-bold">{formatMoney(cost)}</span>}
                  </div>
                )
              })}
            </div>
      ...
      <div className="lg:col-span-2 flex flex-col gap-3 lg:min-h-0">
              ...
              return (
                <div key={i} className="border-b border-[#C2BCB0]/30 last:border-0 pb-3 last:pb-0">
                  <div className="font-display text-lg text-[#1A1D1E]">
                    <span className="text-[#0A2540]">#{i + 1}</span>{' '}
        ...
        {confirmed ? (
          ...
          >
            <Pencil className="w-6 h-6 text-[#0A2540]" /> EDITAR REGISTRO
          </button>
        ) : (
          <>
            <button
            ...
            <button
              onClick={confirm}
              disabled={saving}
              className="shrink-0 h-28 bg-[#0A2540] hover:bg-[#020B14] active:bg-[#020B14] text-white font-display text-2xl tracking-widest rounded-2xl disabled:opacity-50 transition-all shadow-md active:scale-[0.98] duration-150"
            >
        ...
        {saveErr && (
          <p className="shrink-0 text-[#C2410C] text-sm bg-[#C2410C]/10 border border-[#C2410C]/30 rounded-2xl px-3 py-2 break-words font-medium">{saveErr}</p>
        )}
      </div>
    </div>
  )
...
function Card({ title, children, className }: { title: string, children: React.ReactNode, className?: string }) {
  return (
    <div className={`bg-white border border-[#C2BCB0] rounded-2xl p-5 flex flex-col min-h-0 shadow-sm text-[#1A1D1E] ${className ?? ''}`}>
      <p className="text-xs font-display tracking-widest text-[#0A2540] mb-3 shrink-0">{title}</p>
...
function SummaryBlock({ label, children }: { label: string, children: React.ReactNode }) {
  return (
    <div className="bg-white border border-[#C2BCB0] rounded-2xl p-4 shadow-sm">
      <div className="text-xs font-display tracking-widest text-[#0A2540] mb-2">{label}</div>
      {children}
    </div>
  )
}
